...
{
    public class ClaseReporteBoletoResumido
    {
        ...
        string sTipoCliente;
        string sAsientos;
        string sAuxiliarAsiento;
        string sNumeroAsiento;
        ...
        //FUNCION PARA CONSULTAR LOS TIPOS DE CLIENTES
        private bool consultarTipoCliente()
        {
            try
            {
                sSql = "";
                ...
                dtTipoCliente = new DataTable();
                dtTipoCliente.Clear();

                bRespuesta = conexionM.consultarRegistro(sSql, dtTipoCliente);
        ...
        public string llenarBoleto(int iIdPedido_P, int iIdEmpresa)
        {
            try
            {
                ...
                if (consultarTipoCliente() == true)
                {
                    sTexto += "BOLETOS VENDIDOS: " + dtConsulta.Rows.Count.ToString() + Environment.NewLine + Environment.NewLine;

                    iCuenta = 0;
                    sAsientos = "";
                    sAuxiliarAsiento = "";
                    sAuxiliarAsiento += "ASIENTOS: ";

                    for (int j = 0; j < dtConsulta.Rows.Count; j++)
                    {
                        sNumeroAsiento = dtConsulta.Rows[j][60].ToString().Trim();

                        if (sAuxiliarAsiento.Length + sNumeroAsiento.Length > 40)
                        {
                            sAsientos += sAuxiliarAsiento + Environment.NewLine;
                            sAuxiliarAsiento = "";
                            sAuxiliarAsiento += sNumeroAsiento;
                        }
                        ...
                        if (iCuenta != dtConsulta.Rows.Count + 1)
                        {
                            sAuxiliarAsiento += "-";
                        }
                    ...
                    sTexto += sAsientos + Environment.NewLine;

                    for (int i = 0; i < dtTipoCliente.Rows.Count; i++)
                    {
                        iCuenta = 0;
                        iIdTipoCliente = Convert.ToInt32(dtTipoCliente.Rows[i][0].ToString());
                        sTipoCliente = dtTipoCliente.Rows[i][1].ToString().Trim() + ":";

                        for (int j = 0; j < dtConsulta.Rows.Count; j++)
                        {
                            if (iIdTipoCliente == Convert.ToInt32(dtConsulta.Rows[j][73].ToString()))
                            {
                                iCuenta++;
                            }
                        }
                        ...
                        if (iCuenta > 0)
                        {
                            sTexto += sTipoCliente.PadRight(18, ' ') + iCuenta.ToString() + Environment.NewLine;
                        }
                    }
                }
                ...
                {
                    sTexto += "ERROR AL DETALLAR LOS BOLETOS VENDIDOS" + Environment.NewLine;
                }
